...
get_company_info <- function(.html) {
  # Company Name ----------------------------------------------------------------------
  vendor_name_ <- rvest::html_elements(.html, ".vendor_name")

  # Additional Company Information ----------------------------------------------------
  tcl_ <- rvest::html_elements(.html, ".top_content_list")
  ticker_name_ <- rvest::html_elements(tcl_, ".ticker_name")
  exchange_name_ <- rvest::html_elements(tcl_, ".right")

  # Employees and Location ------------------------------------------------------------
  employees_ <- rvest::html_elements(.html, ".employees")
  location_ <- rvest::html_elements(.html, ".location")

  # Description, Website and Social Media ---------------------------------------------
  company_description_ <- rvest::html_elements(.html, ".company_description")
  company_website_ <- rvest::html_elements(.html, ".btn_visit_website") %>%
    rvest::html_element("a") %>%
    rvest::html_attr("href")
  social_media_ <- rvest::html_elements(.html, ".social_media") %>%
    rvest::html_elements("a") %>%
    rvest::html_attr("href")

  facebook_ <- social_media_[grepl("(?i)facebook", social_media_)]
  youtube_  <- social_media_[grepl("(?i)youtube", social_media_)]
  linkedin_ <- social_media_[grepl("(?i)linkedin", social_media_)]
  twitter_  <- social_media_[grepl("(?i)twitter", social_media_)]

  out_ <-
    tibble::tibble(
      vendor_name = unique(rvest::html_text2(vendor_name_)),
      ticker_name = unique(rvest::html_text2(ticker_name_)),
      exchange_name = unique(rvest::html_text2(exchange_name_)),
      employees = unique(rvest::html_text2(employees_)),
      location = unique(rvest::html_text2(location_)),
      description = unique(rvest::html_text2(company_description_)),
      company_website = company_website_,
      facebook = ifelse(length(facebook_) > 0, facebook_, NA_character_),
      youtube = ifelse(length(youtube_) > 0, youtube_, NA_character_),
      linkedin = ifelse(length(linkedin_) > 0, linkedin_, NA_character_),
      twitter = ifelse(length(twitter_) > 0, twitter_, NA_character_),
    )
...
Download the first 10 reports

```{r}
purrr::walk(tab_company_reports$report_link[1:10], ~ download_pdf(.x, "day2/pdfs"), .progress = TRUE)
```
...
download_sec_master_index <- function(.year, .quarter, .dir) {
  # Validate the input
  if (!(.year %in% 1993:as.numeric(format(Sys.Date(), "%Y")))) {
    stop("Invalid year. The EDGAR system started in 1993.")
  }
  ...
  # Define the base URL for the EDGAR master index
  base_url <- "https://www.sec.gov/Archives/edgar/full-index"

  # Construct the URL for the specific .year and quarter
  url <- paste0(base_url, "/", .year, "/QTR", .quarter, "/master.idx")


  # Define the destination path
  dest_path <- file.path(.dir, paste0("master_", .year, "_QTR", .quarter, ".idx"))

  if (!file.exists(dest_path)) {
    # Download the master index using httr::GET
    response <- httr::GET(url, httr::add_headers(Connection = "keep-alive", `User-Agent` = "Matthias"))

    if (response$status_code == 200) {
      writeBin(httr::content(response, "raw"), dest_path)
      return(dest_path)
    } else {
      stop("Failed to download the master index. HTTP status code: ", response$status_code)
    }
  }
}

# Example usage:
download_sec_master_index(2022, 1, .dir = "day2/sec/master")
```
...
tab_master <- read_master_idx("day2/sec/master/master_2022_QTR1.idx")
tab_master
```
...
```{r}
#| echo: false
download_full_submission_file <- function(.tab, .cik, .year, .dir) {
  # Filter the dataframe based on CIK and year
  filtered_rows <- .tab[.tab$cik == .cik & as.character(lubridate::year(.tab$date_filed)) == .year, ]

  # Identify the 10-K submission row
  submission_row <- filtered_rows[filtered_rows$form_type == "10-K", ]

  # Check if a 10-K submission exists for the given CIK and year
  if (nrow(submission_row) == 0) {
    stop("No 10-K submission found for the given CIK and year.")
  }

  # Extract the URL for the 10-K submission
  base_url <- "https://www.sec.gov/Archives/"
  submission_url <- paste0(base_url, submission_row$filename)

  if (!dir.exists(.dir)) dir.create(.dir)


  # Define the destination path
  dest_path <- paste0(.dir, .cik, "_", .year, "_10K.txt")

  if (!file.exists(dest_path)) {
    # Download the 10-K submission
    response <- httr::GET(submission_url, httr::add_headers(Connection = "keep-alive", `User-Agent` = "Matthias"))

    if (response$status_code == 200) {
      writeBin(httr::content(response, "raw"), dest_path)
      return(dest_path)
    } else {
      stop("Failed to download the 10-K submission. HTTP status code: ", response$status_code)
    }
  }
}


download_full_submission_file(tab_master, "1000228", "2022", "day2/sec/txt")
...
extract_section <- function(.path, .section = "Item 1A. Risk Factors") {
  ...
  # Define a regular expression pattern to capture the section
  # This pattern looks for the section name, captures content until the next "Item" section
  pattern_start <- paste0(.section)
  pattern_end <- "Item [0-9]+[A-Z]?\\."
  
  # Find the start and end of the section
  start_pos <- regexpr(pattern_start, content_str)
  end_pos <- regexpr(pattern_end, content_str)
  
  # Extract the section using the start and end positions
  if (start_pos != -1 && end_pos != -1) {
    extracted_section <- substr(content_str, start_pos, end_pos - 1)
  } else {
    extracted_section <- NULL
  }
